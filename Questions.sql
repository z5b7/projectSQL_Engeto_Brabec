...
WITH annual_payroll AS (
    -- výpočet průměrné roční mzdy pro každé odvětví
	SELECT
		"year",
		industry_name,
...
SELECT
	t_current."year" AS year_of_drop,
...
JOIN annual_payroll t_prev
    -- spojení s daty z předchozího roku ve stejném odvětví
	ON t_current.industry_name = t_prev.industry_name
	AND t_current."year" = t_prev."year" + 1
WHERE
    -- filtrace pouze na poklesy mezd
...
SELECT
	...
	ROUND(AVG(((t1.avg_price - t2.avg_price) / t2.avg_price)::NUMERIC * 100), 2) AS average_annual_increase_percent
...
GROUP BY
	t1.food_category_name
ORDER BY
	average_annual_increase_percent ASC 
LIMIT 1; 
...
SELECT
    ...
    ROUND(((t1.avg_price - t2.avg_price) / t2.avg_price)::NUMERIC * 100, 2) AS price_increase_percent, 
    ROUND(((t1.avg_salary - t2.avg_salary) / t2.avg_salary)::NUMERIC * 100, 2) AS salary_increase_percent,   
    -- výpočet rozdílu
    (
